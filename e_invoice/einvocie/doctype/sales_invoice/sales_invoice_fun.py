...
def get_invoice_json(invoice, company, setting, customer):
    ...
    doc.issuer.name = invoice.customer_full_name if len(str(invoice.customer_full_name or " ")  ) > 2 else  clear_str(setting.company_name or company.company_name)
        ...
        if item.item_tax_template:
            ...
            for tax in getattr(tax_template, 'taxes', []):
                ...
                if tax_type and tax_subtype:
                    ...
                    if invoice.is_return == 1:
                        tax_row.rate = round_double(
                        0 if fixed_amount else (tax.tax_rate * -1 ))

                        row_tax = tax.amount if fixed_amount else (
                        (   base_rate_after_discount + (invoice_line.totalTaxableFees / qty)) * (tax.tax_rate * -1)/100)
